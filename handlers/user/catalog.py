...
async def process_catalog(message: Message):
    await message.answer('⛔ <b>Semua Informasi Akan Muncul Dibawah:</b>',
...
async def category_callback_handler(query: CallbackQuery, callback_data: dict):
    ...
    await query.answer('Semua Informasi yang tersedia.')
...
async def add_product_callback_handler(query: CallbackQuery, callback_data: dict):
    ...
    await query.answer('Item ditambahkan ke troli!')
...
async def show_products(m, products):
    ...
    if len(products) == 0:
        ...
        await m.answer('Tidak ada apa-apa di sini 😢')
